...
#' @param all whether to check the IBD for all sample pairs (including the benchmark samples). The default is FALSE. 
...
IBDRemove <- function(seqfile, all = FALSE){
    ...
    if(nrow(ibd.related) > 0){
        ...
        remove.multi <- c()
        repeat{
            ibd.multi <- table(c(as.character(ibd.related.tmp$id1), as.character(ibd.related.tmp$id2)))
            ...
            ibd.related.tmp <- ibd.related.tmp[!ibd.related.tmp$id1 %in% remove.multi & !ibd.related.tmp$id2 %in% remove.multi, ]
